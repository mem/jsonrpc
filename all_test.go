...
//
// Modified from the original test file to use a mapping in all the
// tests involving the server.
...
func TestServerNoParams(t *testing.T) {
	cli, srv := net.Pipe()
	defer cli.Close()
	go ServeConn(srv, testMapping)
	dec := json.NewDecoder(cli)

	fmt.Fprintf(cli, `{"method": "add", "id": "123"}`)
...
func TestServer(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		fmt.Fprintf(cli, `{"method": "add", "id": "\u%04d", "params": [{"A": %d, "B": %d}]}`, i, i, i+1)
		var resp ArithAddResp
		err := dec.Decode(&resp)
		if err != nil {
...
func TestClient(t *testing.T) {
	...
	args = &Args{7, 8}
	mulReply := new(Reply)
	mulCall := client.Go("mul", args, mulReply, nil)
	addReply := new(Reply)
	addCall := client.Go("add", args, addReply, nil)
	...
	args = &Args{7, 0}
	reply = new(Reply)
	err = client.Call("div", args, reply)
...
func TestMalformedInput(t *testing.T) {
	...
	ServeConn(srv, testMapping)    // must return, not loop
}
...
func TestServerErrorHasNullResult(t *testing.T) {
	...
	sc := NewServerCodec(struct {
		...
		Reader: strings.NewReader(`{"method": "add", "id": "123", "params": []}`),
		Writer: &out,
